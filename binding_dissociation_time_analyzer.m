function varargout = binding_dissociation_time_analyzer(varargin)
% BINDING_DISSOCIATION_TIME_ANALYZER MATLAB code for binding_dissociation_time_analyzer.fig
%      BINDING_DISSOCIATION_TIME_ANALYZER, by itself, creates a new BINDING_DISSOCIATION_TIME_ANALYZER or raises the existing
%      singleton*.
%
%      H = BINDING_DISSOCIATION_TIME_ANALYZER returns the handle to a new BINDING_DISSOCIATION_TIME_ANALYZER or the handle to
%      the existing singleton*.
%
%      BINDING_DISSOCIATION_TIME_ANALYZER('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BINDING_DISSOCIATION_TIME_ANALYZER.M with the given input arguments.
%
%      BINDING_DISSOCIATION_TIME_ANALYZER('Property','Value',...) creates a new BINDING_DISSOCIATION_TIME_ANALYZER or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before binding_dissociation_time_analyzer_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to binding_dissociation_time_analyzer_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help binding_dissociation_time_analyzer

% Last Modified by GUIDE v2.5 02-Oct-2018 18:12:44

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @binding_dissociation_time_analyzer_OpeningFcn, ...
                   'gui_OutputFcn',  @binding_dissociation_time_analyzer_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before binding_dissociation_time_analyzer is made visible.
function binding_dissociation_time_analyzer_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to binding_dissociation_time_analyzer (see VARARGIN)

% Choose default command line output for binding_dissociation_time_analyzer
...
% UIWAIT makes binding_dissociation_time_analyzer wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = binding_dissociation_time_analyzer_OutputFcn(hObject, eventdata, handles) 
...
if isempty((data(1))) || isnan((data(1)))
...
elseif peakFinder == "w/o_fitting"
    [file_name,dirr] = uigetfile('*.csv');
    set(handles.edit4, 'String', strcat(dirr, file_name));
    addpath(dirr);
    data_table = table2array(readtable(file_name));
    size(data_table);
    x_data = data_table(:,1);
    y_data = data_table(:,2);
    stacked_image = zeros(512);
...
axes(handles.axes5);
hold off;
loglog(nearfarintensity(1,:), nearfarintensity(2,:), 'r.');
xlabel('near_mean');
ylabel('far_mean');
...
global dirr
global file_name
cur_frame = str2double(get(handles.edit2, 'String'));
csv_name = strcat(dirr, file_name(1:end-4), '_', num2str(cur_frame),'_trace.csv');
csvwrite(csv_name, trace(cur_frame, :));
...
global file_name
cur_frame = str2double(get(handles.edit2, 'String'));
mat_name = strcat(dirr, file_name(1:end-4), '_', num2str(cur_frame),'_trace.mat');
tmp = trace(cur_frame, :);
save(mat_name, 'tmp');
